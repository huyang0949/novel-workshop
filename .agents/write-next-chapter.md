...
根据 `workflow/working/chapter-intent.md` 生成本章上下文包，并输出章节初稿到 `perigee/drafts/`。
...
3. 生成或刷新 `workflow/working/context-pack.md`，只保留本章需要的事实、人物、组织、能力、场景状态和伏笔。
...
- 不直接改 `perigee/canon/`。
- 不把 `workflow/forecasts/` 的候选路线当成已发生事实。
